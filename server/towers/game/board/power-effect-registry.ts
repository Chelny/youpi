...
import { GridPowerHelper } from "@/server/towers/game/board/grid-power-helper";
...
import { SpecialDiamond, SpecialDiamondPowerType } from "@/server/towers/game/pieces/special-diamond/special-diamond";
import { TowersPieceBlock } from "@/server/towers/game/pieces/towers/towers-piece-block";
import { PowerBar, PowerBarItem } from "@/server/towers/game/power-bar";
...
export class AddRowEffect implements PowerEffect<TowersPieceBlock> {
  apply(ctx: PowerEffectContext): void {
    ...
    const grid: BoardGrid = cloneBoardGrid(ctx.grid);
    grid.shift();
    grid.push(Array.from({ length: BOARD_COLS }, () => new TowersPieceBlock()));
    commitBoardGrid(ctx, grid);
...
export class DitherEffect implements PowerEffect<TowersPieceBlock> {
  apply(ctx: PowerEffectContext, item: TowersPieceBlock): void {
    ...
    const grid: BoardGrid = cloneBoardGrid(ctx.grid);
    const gridPowerHelper: GridPowerHelper = new GridPowerHelper(grid);

    const swapCount: number = gridPowerHelper.getNumBlocksToRearrange(item.powerLevel);
    ...
    for (let i = 0; i < swapCount; i++) {
      ...
      while (attempts < MAX_ATTEMPTS_PER_SWAP) {
        ...
        if (gridPowerHelper.isSettingUpThreeInRow(r1, c1, r2, c2)) continue;

        // Don’t create adjacent same-color pairs (your extra rule)
        if (gridPowerHelper.areAdjacentBlocksSame(r1, c1, r2, c2)) continue;

        // We found a valid swap
        gridPowerHelper.swapBlocks(r1, c1, r2, c2);
        break;
      }
...
export class ClumpEffect implements PowerEffect<TowersPieceBlock> {
  apply(ctx: PowerEffectContext, item: TowersPieceBlock): void {
    ...
    const clumpCount: number = gridPowerHelper.getNumBlocksToRearrange(item.powerLevel);
    const MAX_ATTEMPTS_PER_SWAP = 50;
    ...
    for (let i = 0; i < clumpCount; i++) {
      ...
      while (attempts < MAX_ATTEMPTS_PER_SWAP) {
        ...
        // Your gridPowerHelper already guarantees this should be false, but keeping this is fine as a safety net:
        if (gridPowerHelper.isSettingUpThreeInRow(r1, c1, r2, c2)) {
          gridPowerHelper.swapBlocks(r1, c1, r2, c2);
          continue;
        }
...
export class DefuseEffect implements PowerEffect<TowersPieceBlock> {
  apply(ctx: PowerEffectContext, item: TowersPieceBlock): void {
    ...
    const gridPowerHelper: GridPowerHelper = new GridPowerHelper(grid);
    const toReplace: number = Math.min(
      gridPowerHelper.getNumBlocksToRearrange(item.powerLevel, purples.length),
      purples.length,
    );
...
/**
 * Creates a shallow clone of a board grid.
 * Each row is copied to avoid mutating the original grid directly.
 *
 * @param grid - The board grid to clone.
 * @returns A new BoardGrid with cloned rows.
 */
function cloneBoardGrid(grid: BoardGrid): BoardGrid {
  return grid.map((row: BoardGridRow) => [...row]);
}

/**
 * Commits a modified board grid to a power effect context.
 * This will replace the current grid in the context with the provided one.
 *
 * @param ctx - The PowerEffectContext containing the current board.
 * @param grid - The new board grid to set in the context.
 */
function commitBoardGrid(ctx: PowerEffectContext, grid: BoardGrid): void {
  ctx.setGrid?.(grid);
}
