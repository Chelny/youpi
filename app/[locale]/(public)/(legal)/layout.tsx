import { ReactNode } from "react";
import { I18n } from "@lingui/core";
import clsx from "clsx/lite";
import { getI18nInstance } from "@/app/app-router-i18n";
import LanguageSwitcher from "@/components/LanguageSwitcher";
import { Footer } from "@/components/layout/Footer";
import { Header } from "@/components/layout/Header";
import Anchor from "@/components/ui/Anchor";
import { ROUTE_HOME } from "@/constants/routes";

type LegalLayoutProps = LayoutProps<"/[locale]">;

export default async function LegalLayout({ params, children }: LegalLayoutProps): Promise<ReactNode> {
  const { locale } = await params;
  const i18n: I18n = getI18nInstance(locale);
  return (
    <div className="grid grid-rows-[max-content_auto_max-content] h-dvh">
      <Header>
        <nav className="flex-1 flex flex-row justify-end items-center gap-2">
          <LanguageSwitcher className="place-self-end" />
          <ul
            className={clsx(
              "hidden md:flex flex-row justify-end divide-x divide-white/50",
              "[&_li]:not-last:pe-2 [&_li]:not-first:not-last:px-2 [&_li]:last:ps-2",
            )}
          >
            <li>
              <Anchor href={ROUTE_HOME.PATH} className={clsx("px-1 text-white", "hover:text-white/50")}>
                {i18n._(ROUTE_HOME.TITLE)}
              </Anchor>
            </li>
          </ul>
        </nav>
      </Header>
      <main className="container mx-auto px-4 py-8 overflow-auto">{children}</main>
      <Footer />
    </div>
  );
}
